...
int process_paf(int p1, int p2, int p3, float *peaks, int h1, int h2, int h3, float *heatmap, int f1, int f2, int f3, float *pafmap) {
    ...
    for (int pair_id = 0; pair_id < COCOPAIRS_SIZE; pair_id ++) {
        ...
        if (peak_a_list.size() == 0 or peak_b_list.size() == 0) { //if(nA == 0 && nB ==0)
            continue;
        }
        ...
        for (int conn_id = 0; conn_id < (int) conns.size(); conn_id ++) {
            ...
            if (found == 1) {
                if (subset[subset_idx1][part_id2] != conns[conn_id].cid2) {
                    ...
                    subset[subset_idx1][16] += 1;
                    subset[subset_idx1][15] += peak_infos_line[conns[conn_id].cid2].score + conns[conn_id].score;
                }
            } else if (found == 2) {
                int membership=0;
                for (int subset_id = 0; subset_id < 15; subset_id ++) {
            ...
            } else if (found == 0 && pair_id < 14) {
                vector<float> row(17);
                for (int i = 0; i < 17; i ++) row[i] = -1;
                row[part_id1] = conns[conn_id].cid1;
                row[part_id2] = conns[conn_id].cid2;
                row[16] = 2;
                row[15] = peak_infos_line[conns[conn_id].cid1].score +
    ...
    for (int i = subset.size() - 1; i >= 0; i --) {
        if (subset[i][16] < THRESH_PART_CNT || subset[i][15] / subset[i][16] < THRESH_HUMAN_SCORE)
...
float get_score(int human_id) {
    return subset[human_id][13] / subset[human_id][14];
}
