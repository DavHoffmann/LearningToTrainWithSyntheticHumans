...
class BaseNetwork(object):
    def __init__(self, inputs, trainable=True, output_size=None, nr_vectmaps=None, nr_keypoints=None, trainVgg=None):
        ...
        if not(nr_vectmaps is None):

            if output_size is None:
                if trainVgg is None:
                    self.setup(nr_vectmaps=nr_vectmaps, nr_keypoints=nr_keypoints)
                else:
                    self.setup(nr_vectmaps=nr_vectmaps, trainVgg=trainVgg, nr_keypoints=nr_keypoints)
            else:
                if trainVgg is None:
                    self.setup(output_size, nr_vectmaps=nr_vectmaps, nr_keypoints=nr_keypoints)
                else:
                    self.setup(output_size, nr_vectmaps=nr_vectmaps, trainVgg=trainVgg, nr_keypoints=nr_keypoints)
        else:
            if output_size is None:
                if trainVgg is None:
                    self.setup()
                else:
                    self.setup(trainVgg=trainVgg)
            else:
                if trainVgg is None:
                    self.setup(output_size)
                else:
                    self.setup(output_size, trainVgg=trainVgg)
        ...
        for op_name in data_dict:
            ...
            if isinstance(data_dict[op_name], np.ndarray):
                ...
                if isinstance(op_name, str):
                    with tf.variable_scope(op_name, reuse=True):
                        ...
                        for param_name, data in data_dict[op_name].items():
                            try:
                                ...
                                print(ae)
                                try:
                                    print('maybe it is not of type byte')
                    ...
                    with tf.variable_scope(op_name.decode('utf-8'), reuse=True):
